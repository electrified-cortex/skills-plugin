#!/usr/bin/env bash
# comment.sh — Post a comment on a GitHub issue via gh issue comment.
...
if ! [[ "$ISSUE" =~ ^[0-9]+$ ]]; then
  printf 'USAGE_ERROR: --issue must be a positive integer, got: %s\n' "$ISSUE" >&2
  exit 2
fi
...
# ---------------------------------------------------------------------------
# Invoke gh issue comment
# ---------------------------------------------------------------------------
# Build argument array — body file passed by path, never interpolated.
# gh issue comment prints the comment URL to stdout on success.
gh_args=(
  issue comment "$ISSUE"
  --repo "${OWNER}/${REPO}"
  --body-file "$BODY_FILE"
